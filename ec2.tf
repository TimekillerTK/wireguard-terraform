...
resource "aws_instance" "vpn_instance" {
  ...
  provisioner "local-exec" {
    command = "sleep 90 && scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ubuntu@${aws_eip.vpn_instance_eip.public_ip}:/wg_client/wg0.conf ${var.output_folder} && sh ./files/post_deploy.sh"
  }
}
